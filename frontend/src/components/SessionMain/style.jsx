import styled from "styled-components";

export const LeaderWrapper = styled.div``;
export const LeaderImg = styled.div``;
export const LeaderInfo = styled.div``;
export const LeaderName = styled.div``;
export const LeaderIntro = styled.div``;
export const LeaderLink = styled.div``;
export const LeaderProfile = styled.div``;

export const SessionBottom = styled.div``;
export const SessionBottomBox = styled.div``;
export const SessionBottomHeader = styled.div``;
export const SessionContainer = styled.div``;
export const SessionCurriExplain = styled.div``;
export const SessionCurriItem = styled.div``;
export const SessionCurriTitle = styled.div``;
export const SessionEdit = styled.div``;
export const SessionHeader = styled.div``;
export const SessionHeaderCont = styled.div``;
export const SessionHeaderItem = styled.div``;
export const SessionImg = styled.div``;
export const SessionIntroExplain = styled.div``;
export const SessionIntroItem = styled.div``;
export const SessionIntroTitle = styled.div``;
export const SessionLevel = styled.div``;
export const SessionTitle = styled.div``;
